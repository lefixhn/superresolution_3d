...
class BasicUNetU(nn.Module): 
    ...
    def __init__(self, upscale_factor=2):
        ...
        self.decoder_level1 = nn.Sequential([
            ...
            nn.Conv3d(in_channels=decoder_level1_out_channels, out_channels=2**upscale_factor), 
        ])
    ...
    def forward(self, x):
        ...
        upscale = F.interpolate(scale_factor=(self.upscale_factor, self.upscale_factor, self.upscale_factor))

        encoder_level1_out = self.encoder_level1(x)
        encoder_level2_out = self.encoder_level2(pool(encoder_level1_out))
        encoder_level3_out = self.encoder_level2(pool(encoder_level2_out))
        ...
        devoder_level_1_in = torch.cat([upscale(decoder_level_2_out), encoder_level1_out])
